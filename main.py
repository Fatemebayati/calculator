...
class calculator(tk.Tk):
    def __init__(self):
        ...
        # self.entry = tk.Entry(self, font=("Arial", 20), borderwidth=5, relief="ridge", justify="right")
        ...
        self.create_button("=",button_frame,3,2,self.calculator,'green')
        self.create_button("C",button_frame,3,0,self.clear)

    # def calculate(self):
    #     try:
    #         result = eval(self.entry.get())  # مقدار داخل entry رو محاسبه می‌کنه
    #         self.entry.delete(0, tk.END)     # پاک کردن متن قبلی
    #         self.entry.insert(tk.END, str(result))  # نمایش نتیجه
